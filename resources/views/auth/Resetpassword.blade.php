    ...
    <div class="container-fluid p-0">
        <div class="card">
            <div class="card-body">
                ...
                <div class="p-3">
                    <form class="form-horizontal mt-3" method="POST" action="{{ route('reset.password.process') }}">
                        @csrf
                        <div class="form-group mb-3 row">
                            <div class="col-12">
                                <input class="form-control" id="otp" type="text" name="otp" required autofocus autocomplete="new otp" placeholder="Enter 6 digit otp" />
                                <label id="otp-error" class="text-danger"></label>
                            </div>
                        </div>
                        ...
                        <div class="form-group mb-3 row">
                            <div class="col-12">
                                <input class="form-control @error('password') is-invalid @enderror" id="password" type="password" name="password"
                                       required autofocus autocomplete="new-password" placeholder="Enter new Password" />
                                @error('password')
                ...
                <strong>{{ $message }}</strong>
                        ...
                        <div class="flex items-center justify-end mt-4">
                            <button type="submit" class="btn btn-info w-100 waves-effect waves-light">Reset Password
                            </button>
                        </div>

                        <div class="form-group mt-2 mb-0 row">
                            <div class="col-12 mt-3 text-center">
                                <a href="{{ route('login') }}">Go back Login</a>
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    $(document).ready(function() {
        $('#otp').on('input', function() {
            ...
            var formattedValue = inputValue.split('').join('-');
            $(this).val(formattedValue);

            $('#otp-error').text(''); // Clear any previous error message
        });
        $('#otp').on('blur', function() {
            $('#otp-error').text('');
            var inputValue = $(this).val().replace(/-/g, ''); // Remove dashes
            if (inputValue.length !== 6) {
